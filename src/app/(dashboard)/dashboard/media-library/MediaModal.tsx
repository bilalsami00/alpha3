...
export default function MediaModalV2({
  ...
  mode?: "add" | "edit";
  initial?: Partial<MediaItem> | null;
  onSave: (payload: { title: string; description?: string; file?: File | null }) => Promise<MediaItem | void> | MediaItem | void;
  onDelete?: () => void;
}) {
  const { showToast } = useToastContext();
  ...
  async function handleSave() {
    if (!canSave) return;
    try {
      const returned = await onSave({ title: title.trim(), description: description.trim() || undefined, file });
      if (returned && returned.mediaUrl) {
        const norm = normalizePath(returned.mediaUrl);
        setPreviewVideoUrl(norm);
        setPreviewImageUrl(returned.thumbnailUrl ?? null);
  ...
  return (
    <BaseModal
      onClose={onClose}
      className="w-full max-w-[438px] mx-auto" /* responsive container — BaseModal will cap to viewport */
    >
      {/* parent uses flex column and caps maxHeight so header/footer stay visible and middle scrolls */}
      <div
        className="media-modal flex flex-col bg-white rounded-xl overflow-hidden"
        style={{ maxHeight: "calc(100vh - 48px)", width: "100%" }}
      >
        {/* Header */}
        ...
        <div className="p-4 sm:p-6 overflow-auto modal-content flex-1 min-w-0">
          <label className="block">
            <div className="txt-14 font-normal text-gray-700 mb-2">Video</div>
            ...
            <div className="w-full relative min-w-0  ">
              <label
                className="
                  dashed-figma rounded-lg cursor-pointer bg-white
                  w-full overflow-hidden relative flex items-center justify-center h-[190px]
                "
              >
                <input ref={inputRef} type="file" accept="video/mp4,video/*" className="hidden" onChange={handleFileChange} />
                ...
                {previewVideoUrl ? (
                  <div className="absolute inset-0 w-full h-full flex items-center justify-center">
                    {/* responsive heights: xs:160, sm:190, md:220, lg:280 */}
                  ...
                  <div className="text-center txt-14 font-normal py-6 sm:py-8">
                    <div className="flex items-center justify-center mb-2">
                      <Image src="/dashboardIcons/MediaLibrary/video-circle.svg" alt="Upload" width={32} height={32} className="mr-2" />
                    </div>
                    <div>Upload an MP4 (1080p) under 500MB.</div>
              ...
              {hasExistingPreview && (
                <button
                  ...
                  onClick={handleRemovePreview}
                  aria-label="Remove video"
                  className="absolute top-3 right-3 z-20 flex items-center justify-center rounded-full bg-white/60 backdrop-blur"
                >
                  <Image src="/CloseButton.svg" alt="Remove" width={20} height={20} />
                </button>
              )}
          ...
          <label className="block mt-4">
            ...
            <input
              ...
              onChange={(e) => setTitle(e.target.value)}
              placeholder="Enter video title"
              className="w-full rounded-md px-3 py-3 bg-[#F3F6F6] border border-transparent placeholder-[#9CA3AF] txt-14 focus:outline-none focus-green"
            />
          </label>
          ...
          <label className="block mt-4">
            ...
            <textarea
              ...
              placeholder="Add Description"
              rows={5}
              className="w-full rounded-md px-3 py-4 bg-[#F3F6F6] border border-transparent placeholder-[#9CA3AF] txt-14 resize-none focus:outline-none focus-green"
            />
          </label>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end gap-3 bg-[#F2F5F6] px-4 sm:px-6 py-3 border-t border-[#E9EDEE]">
          <button onClick={onClose} className="px-3 py-2 txt-14 font-medium rounded-md bg-white border border-[#E9EDEE]">
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={!canSave}
            className="px-3 py-2 txt-14 font-semibold rounded-md bg-[#0E0E0E] text-white disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {mode === "add" ? "Add" : "Save Changes"}
          </button>
        </div>
