// // src\app\(dashboard)\dashboard\media-library\guided-breathwork\GuidedBreathwork.tsx
// "use client";
...
export default function GuidedBreathwork({
  ...
  function openEdit(item: MediaItem) {
    setModalMode("edit");
    // lookup fresh item from state to avoid stale object reference
  ...
  async function handleSave(payload: {
    ...
    if (modalMode === "edit" && editing) {
      // if we're replacing the file, revoke previous blob URL if any
      ...
      if (payload.file && editing.mediaUrl && editing.mediaUrl.startsWith("blob:")) {
        // revoke old blob before replacing
        try {
