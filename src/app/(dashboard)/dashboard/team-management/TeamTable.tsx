...
// (keep the call if you rely on it elsewhere — original file had it)
closeAllRowActionMenus();
...
type Props = {
  ...
  // NEW: pass total count (before pagination) so the table can show an empty state inside
  totalItems?: number;
  // NEW: optional action node (e.g. create button) that will be shown inside the EmptyState
...
export default function TeamTable({
  ...
  // Build the EmptyState node once — shown inside the table when there are no items
  ...
  return (
    <GenericTable<Team>
      ...
      data={data}
      rowKey={(team) => team.id}
      // show the empty state inside the table body when totalItems === 0
      emptyState={totalItems === 0 ? tableEmptyState : null}
      rowClassName={(r, idx) =>
        (startIndex + idx) % 2 === 0 ? "bg-white" : "bg-[#F2F5F6]"
      }
    />
