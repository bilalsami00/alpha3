// // // // src\app\(dashboard)\dashboard\sales-manual\reps-checklist\page.tsx
// "use client";
...
// import AddEditSalesModal from "../AddEditSalesModal"; // <-- import modal for fallback
...
//   onAdd?: () => void;
//   onEdit?: (item: { category: string; task: string }) => void;
// }) {
...
//           const next = [...p];
//           value.forEach((r: any) => {
//             const idx = next.findIndex((x) => x.task === r.task);
//             if (idx !== -1) next[idx] = { ...next[idx], category: r.category, task: r.task };
//             else
//               next.unshift({
//                 id: Date.now(),
//                 category: r.category,
...
//         <ActionMenu
//           onEdit={() => {
//             console.log("RepsChecklist: edit clicked", r);
//             if (onEdit) {
//               onEdit({ category: r.category, task: r.task });
//             } else {
//               // fallback: open local modal and also dispatch same custom event so other listeners can react
//               console.log("RepsChecklist: onEdit prop missing — opening fallback local modal and dispatching salesManual:itemSaved after save");
//               setFallbackMode("edit");
//               setFallbackInitial({ category: r.category, task: r.task });
//               setFallbackModalOpen(true);

//               // also dispatch a lightweight request event for other listeners if needed
//               document.dispatchEvent(
//                 new CustomEvent("salesManual:requestEdit", {
//                   detail: { type: "repsChecklist", item: { category: r.category, task: r.task } },
//                 })
//               );
...
//     showToast("Reps checklist deleted.", "info");
...
//   return (
//     <div>
//       {/* If parent didn't render its own header/add button (no onAdd prop), show a local header so user still sees it */}
//       {!onAdd && (
...
//         }}
//         onSave={(payload) => {
//           // dispatch the same global event your other components expect so everything updates consistently
//           const detail = { type: "repsChecklist", mode: fallbackMode, value: Array.isArray(payload) ? payload : [payload] };
//           document.dispatchEvent(new CustomEvent("salesManual:itemSaved", { detail }));
//           // also ensure local list updates (the document listener above will handle it, but keep this defensive)
//           if (Array.isArray(payload)) {
//             setItems((p) => [...payload.map((r: any) => ({ id: Date.now() + Math.floor(Math.random() * 1000), category: r.category || "Uncategorized", task: r.task || "" })), ...p]);
//           } else if (payload && typeof payload === "object") {
//             setItems((p) => [{ id: Date.now(), category: payload.category || "Uncategorized", task: payload.task || "" }, ...p]);
//           }
//           setFallbackModalOpen(false);
//         }}
//       />
...
import RepsChecklist from "./RepsChecklist";

export default function Page() {
  return <RepsChecklist />;
}
