...
//             new CustomEvent("salesManual:itemSaved", { detail })
//           );
...
//         if (mode === "edit" && initial?.id !== undefined)
...
//                         onClick={goNext}
//                         disabled={!canNext}
//                         className={`p-1.75 rounded-full border border-[var(--Neutral-Grey-10,#E9EDEE)] ${
//                           !canNext ? "opacity-40" : ""
//                         }`}
...
//                   placeholder="Enter sales term"
//                   className="w-full p-3 rounded bg-[#F2F5F6] txt-14 focus:outline-none"
//                 />
//               </div>

//               <div>
//                 <div className="txt-12 font-medium mb-2">
//                   Fundamental Script
//                 </div>
//                 <textarea
//                   value={script}
//                   onChange={(e) => setScript(e.target.value)}
//                   placeholder="Add fundamental script"
//                   rows={6}
//                   className="w-full p-3 rounded bg-[#F2F5F6] txt-14 resize-none focus:outline-none"
//                 />
//               </div>
//             </>
//           )}
...
//           <button
//             onClick={onClose}
//             className="w-28 h-10 px-3 txt-16 font-semibold rounded-lg bg-white border border-[color:var(--Neutral-Grey-10,#E9EDEE)]"
//           >
//             Cancel
//           </button>
//           <button
...
//             className="w-36 h-10 px-3 txt-16 font-semibold rounded-lg bg-[#0E0E0E] text-white disabled:opacity-50 disabled:cursor-not-allowed"
//           >
//             {saving
//               ? mode === "edit"
//                 ? "Saving…"
...
//             value={e.task}
//             onChange={(ev) =>
//               onUpdate({ task: ev.target.value.slice(0, maxChars) })
//             }
...
export default function AddEditSalesModal({
  ...
  // responsive carousel measurement
  const trackRef = useRef<HTMLDivElement | null>(null);
  const cardOuterRef = useRef<HTMLDivElement | null>(null);
  const [cardWidth, setCardWidth] = useState<number>(CARD_DESKTOP);
  ...
  useLayoutEffect(() => {
    function measure() {
      ...
      if (outer) {
        ...
        setCardWidth(Math.min(CARD_DESKTOP, available));
        return;
      }
      ...
      setCardWidth(Math.min(CARD_DESKTOP, typeof window !== "undefined" ? window.innerWidth - 48 : CARD_DESKTOP));
    }
  ...
  function handleAddAnother() {
    if (!text.trim() || !selectedCategory) return;
    const uid = `${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    const entry: ChecklistEntry = { uid, category: selectedCategory, task: text.trim() };
    ...
    window.setTimeout(() => setAnimateUid(null), 438);
  ...
  async function handleSave() {
    if (isChecklist) {
      ...
      const all = [...pending.map((p) => ({ category: p.category, task: p.task })), ...entries.map((e) => ({ category: e.category, task: e.task }))];
      ...
      try {
        if (mode === "edit") {
          const single = valid[0];
          const detail = { type: "repsChecklist", mode: "edit", value: [{ ...(single as any), id: initial?.id ?? undefined }], eventId };
      ...
      try {
        ...
        const detail = { type: "fundamentalScript", mode, value: payload, initialValue: initial, eventId };
        document.dispatchEvent(new CustomEvent("salesManual:itemSaved", { detail }));
        if (onSave) await onSave(payload);
      } finally {
        setSaving(false);
  ...
  return (
    <BaseModal onClose={onClose} className="w-full max-w-[486px] mx-auto">
      ...
      >
         <style>{`
    @keyframes slideIn {
      0% { opacity: 0; transform: translateX(40px) translateY(-6px) scale(.995); }
      60% { opacity: 1; transform: translateX(-8px) translateY(2px) scale(1.01); }
  ...
  `}</style>
        ...
        <div ref={scrollContainerRef} className="p-4 sm:p-6 overflow-auto min-h-0 flex-1">
          {isChecklist ? (
            <>
              <div className="mb-4 flex flex-col gap-4">
                <div>
                ...
                <div>
                  ...
                  {compact ? (
                    <input
                      ...
                      onChange={(e) => setText(e.target.value)}
                      placeholder="Enter checklist task"
                      className="w-full p-3 rounded bg-[#F2F5F6] txt-14 focus:outline-none placeholder:text-14 h-14 txt-12"
                    />
                  ) : (
                    <textarea
                      ref={textRef as any}
        ...
        <div className="flex items-center justify-end gap-4 bg-[#F2F5F6] px-4 sm:px-6 py-3 border-t border-[color:var(--Neutral-Grey-20,#E9EDEE)]">
...
function PreviewCard({
  ...
  //   <div className={`bg-white rounded-lg p-3 border w-full  border-[var(--Neutral-Grey-10,#E9EDEE)]`} style={isAnimating ? { animation: "slideInX 380ms cubic-bezier(.2,.9,.25,1)", animationFillMode: "both" } : {}}>
      return (
  <div
    className={`bg-white rounded-lg p-3 border w-full sm:w-[438px] border-[var(--Neutral-Grey-10,#E9EDEE)]`}
    style={isAnimating ? { animation: "slideIn 380ms cubic-bezier(.2,.9,.25,1)", animationFillMode: "both" } : {}}
  >
<div className="ml-2 flex items-end justify-end">
