...
// // //   // derive unique list of teams from TEAM_MEMBERS (flexible to different shapes)
// // //   const teams = useMemo(() => {
...
// // //         (m as any).teamName ||
// // //         (m as any).team ||
// // //         ((m as any).team && (m as any).team.name) ||
// // //         "Unknown Team";
// // //       set.add(name);
// // //     });
...
// // //         (member as any)?.team ||
// // //         ((member as any)?.team && (member as any).team.name) ||
// // //         "";
...
// // //       const matchesTeam = !selectedTeam || teamName === selectedTeam;

// // //       return matchesQuery && matchesTeam;
// // //     });
// // //   }, [items, searchQuery, selectedTeam]);
...
// // //   // helper to get team name from a member object (robust)
// // //   const getTeamNameFromMember = (m?: TeamMember | null) => {
// // //     if (!m) return "Unknown Team";
// // //     return (
// // //       (m as any).teamName ||
// // //       (m as any).team ||
// // //       ((m as any).team && (m as any).team.name) ||
...
// // //           username: "",
// // //         };
// // //         const teamName = getTeamNameFromMember(m as TeamMember);

// // //         return (
// // //           <div className="flex items-center gap-3">
// // //             <div className="px-3 py-1 rounded-full bg-[#F2F5F6] txt-12 font-medium">
// // //               {teamName}
// // //             </div>
// // //           </div>
// // //         );
...
// //   const getTeamNameFromMember = (m?: TeamMember | null) => {
// //     if (!m) return "Unknown Team";
// //     // prefer numeric teamId mapping first if present
// //     if ((m as any).teamId) {
// //       const team = INITIAL_TEAMS.find((t) => t.id === Number((m as any).teamId));
...
// //           username: "",
// //         };
// //         // prefer mapping via numeric teamId if present
// //         let teamName = "";
// //         if ((m as any)?.teamId != null) {
// //           teamName =
// //             INITIAL_TEAMS.find((t) => t.id === Number((m as any).teamId))?.name ||
// //             "";
// //         }
// //         if (!teamName) teamName = getTeamNameFromMember(m as TeamMember);
...
//     // 1) Try explicit numeric team id on member
//     const memberTeamId = (m as any)?.teamId ?? (m as any)?.team?.id ?? null;
//     const rowTeamId = (row as any)?.teamId ?? null;
//     const teamId = memberTeamId ?? rowTeamId;

//     if (teamId != null && teamId !== "") {
//       const t = INITIAL_TEAMS.find((tt) => tt.id === Number(teamId));
//       if (t) return t.name;
//     }

//     // 2) Try common team name fields on member
//     const candidates = [
//       (m as any)?.teamName,
//       (m as any)?.team,
//       (m as any)?.team?.name,
...
//     // 3) Try to infer from TEAM_MEMBERS: if some members include a team and share some property (not guaranteed)
//     // This is a best-effort heuristic: if the member object has a "group" or "org" field that matches other members who do have team info, pick that.
//     try {
//       const fallbackKeyCandidates = ["group", "org", "department", "unit"] as const;
//       for (const key of fallbackKeyCandidates) {
//         const val = (m as any)?.[key];
//         if (!val) continue;
//         const match = TEAM_MEMBERS.find((tm) => {
//           const tmVal = (tm as any)[key];
//           return tmVal && String(tmVal) === String(val) && ((tm as any).teamName || (tm as any).team || (tm as any).teamId);
//         });
//         if (match) {
//           const name = (match as any).teamName || (match as any).team || ((match as any).teamId ? (INITIAL_TEAMS.find(t => t.id === Number((match as any).teamId))?.name) : undefined);
...
//     // final fallback: return empty string so UI shows a neutral placeholder (instead of 'Unknown Team')
//     return "";
//   };
